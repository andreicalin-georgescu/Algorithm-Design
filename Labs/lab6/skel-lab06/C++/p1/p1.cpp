...
Move::Move(int amount, int heap): amount(amount), heap(heap)
{
}

Move::Move(): amount(0), heap(-1)
{
}
...
Nim::eval(int player)
{
     ...
     */
    int sum = heaps[0] + heaps[1] + heaps[2];
    ...
    return heaps[0] + heaps[1] + heaps[2]; // castigul maxim al playerului
...
minimax(Nim init, int player, int depth)
{
    ...
    for (Move move : moves) {
        ...
        if (!new_nim.apply_move(move)) {
            continue;
        }

        int score = -minimax(new_nim, -player, depth - 1).first;
...
minimax_abeta(Nim init, int player, int depth, int alfa, int beta)
{
    ...
    for (Move move : moves) {
        ...
        score = -minimax_abeta(new_nim, -player, depth - 1, -alfa, -beta).first;
    ...
    return std::pair<int, Move>(score, valid_move);
}
